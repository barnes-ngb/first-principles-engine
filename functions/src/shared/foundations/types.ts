 ...
 *
 * THE definition lives here, in `functions/src/shared/`, compiled by BOTH the app
 * and the Cloud Functions projects (the ARCH-47 pattern — UX-296). It moved here
 * unchanged from `src/core/foundations/types.ts`, which keeps its path and
 * re-exports, so every consumer is untouched.
 *
 ...
 * seeder (`src/core/foundations/seedLearnerModel.ts`) is the only thing that turns
 * this graph plus a child's derived signals into a stored `LearnerModel`; it stays
 * on the app side, because it reads app-typed snapshots.
 */
