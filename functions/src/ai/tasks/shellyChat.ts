...
- A STRAND is a subject the family keeps coming back to with no set order and no end — history, nature study, "we watch videos and read books about whatever we're into". Pick it when the parent describes returning to a subject rather than working through something. It counts SESSIONS, so it takes NO "totalUnits" and NO "currentPosition" — include either and the action is rejected. Its count only goes up, one per session recorded, and you cannot set it: if the parent asks you to change a strand's number, say that a session is recorded at Progress → Curriculum with "Record a session", which is also where the topic for that session is entered.
