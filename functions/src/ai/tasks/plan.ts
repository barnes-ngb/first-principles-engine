...
export function buildPlannerBoundarySection(): string {
  ...
  return [
    ...
    "From here you can only shape the WEEK'S DRAFT PLAN — which items sit on which day, their",
    "order and their minutes. Nothing else you say reaches the app. In particular you CANNOT change:",
    ...jobLines,
    "",
    "- NEVER say you have made one of those changes, and NEVER say one is coming — you cannot know,",
    "  and she plans around what you tell her.",
    "- NEVER name a screen, tab, setting or menu of your own. Inventing a plausible-sounding location",
    "  is worse than admitting the gap, because she will go looking for something that isn't there.",
    "  Do NOT write a URL or a path either. The marker below is how you point — the app draws the button.",
    "- When she asks for one of those jobs: say plainly you can't do it from here, then say in the same",
    "  breath what you CAN do for the week instead, and end your reply with EXACTLY ONE marker on its",
    "  own final line, naming the closest job id from the list above:",
    `    ${PLANNER_BOUNDARY_MARKER_EXAMPLE}`,
    "  If none of the ids above fits what she asked for, write the marker with no id at all:",
    `    ${PLANNER_BOUNDARY_MARKER_UNPLACED}`,
    "  Never guess an id that isn't in the list — a wrong destination is worse than a general one.",
    "- Emit the marker ONLY when you are declining one of those jobs. Never on an ordinary reply, never",
    "  more than once, and NEVER anywhere inside a JSON plan response.",
  ].join("\n");
}
...
export const handlePlan = async (
  ...
  // Knowledge Mine is NOT a checklist item — it's an always-available tool on Kid Today
  // that auto-tracks time to hours. Only Fluency Practice is scheduled by the planner.
  sections.push([
    "EVALUATION SCHEDULING:",
    "Include Fluency Practice as a checklist item in the weekly plan.",
    "Do NOT schedule Knowledge Mine items — Knowledge Mine is an always-available tool on",
    "Kid Today (not a checklist task) and auto-tracks its own session time to hours.",
    "",
    "Based on the child's Skill Snapshot and current focus areas:",
    "",
    "IF reading has emerging/not-yet skills AND fluency is a focus:",
    '  → Schedule 2 "Fluency Practice" sessions per week (10m each)',
    "  → Alternate days — don't bunch them together",
    "",
    "Spread evaluation sessions across the week. Maximum 1 fluency per day.",
  ...
  sections.push([
    ...
    'Fluency items get block: "independent".',
    "Note: Knowledge Mine is NOT scheduled here — it's an always-available tool on Kid Today",
    "that auto-tracks its own time.",
    "",
    "ORDERING RULES:",
  ...
  sections.push([
    ...
    "(Knowledge Mine is NOT scheduled — it's an always-available tool on Kid Today.)",
    "",
    "ROTATION RULES:",
    ...
    "Monday: Formation → Read-Aloud+Handwriting → Choice(both) → GATB Reading → GATB Math → Sight Words",
    "Tuesday: Formation → Read-Aloud+Handwriting → Choice(both) → GATB Reading → GATB Math → Memory Card → Fluency Practice",
    "Wednesday: Formation → Read-Aloud+Handwriting → Choice(both) → GATB Reading → GATB Math → Sight Words → Fluency Practice",
    "Thursday: Formation → Read-Aloud+Handwriting → Choice(both) → GATB Reading → GATB Math → Memory Card",
