...
A) A CURRICULUM-APP SCREENSHOT (e.g. "these are Fast Phonics" — a progress page, peak/lesson list, quiz results):
   ...
   - DID-YOU-MEAN: if the parent's named source looks like a near-miss SPELLING of a bridged source below (e.g. "Fast phony", "Fast fonics", "fastphonic" for **Fast Phonics**), do NOT silently treat it as an unknown generic source. Propose the generic \`covered\` AND, in your prose, gently ask "did you mean Fast Phonics? — if so I can line these up against the Fast Phonics peaks." A one-line confirmation lets the next turn map it precisely. Never drop the mapping over a typo.
