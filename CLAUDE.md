...
- `src/features/books/` — Bookshelf, book editor/reader, generate chat, review chat, sight word dashboard, story guide, print/PDF. Generate Chat's word channel (FEAT-169/172) lives in `storyPracticeWords.ts`: **a list the parent typed into the idea wins** (`parseRequestedWords` → `resolveStoryWords`), the child's `practicing`/`new` words are the fallback, and both confirmation lines name the source. The chat binds every read and write to **the child the story is FOR** (a parent's "This story is for" picker in `BookGenerateChat`, hydrated from the book on resume), never to the header's active child. Read-only on `sightWordProgress`; `CreateSightWordBook`'s typed-list path is separate and untouched
