...
| **Tests (root)** | ❌ **1 FAILING** | 7,788 passing, 1 failing, 0 skipped (537 test files, `src/` + `functions/src/` combined via root `vitest.config.ts`). See **CRITICAL finding** below — this is a test-infrastructure gap exposed only by a full-history clone, not an app regression, and CI never sees it (CI checks out shallow). |
...
**Not a regression from anything in this run** — the ledger content is untouched by this session, and the "holds on the live ledger" sibling test (same file) **passes**. This is a latent gap in the checker's pattern-matching precision that only a full-history run can see, and the project's own tooling deliberately prevents CI from ever seeing it. Per this audit's failure-mode policy, **not fixed** — flagged for human attention. Two options for a human: (1) accept this as an intentional, CI-invisible probe and leave it (re-running it locally with full history will always show this until the FEAT-112 pattern is special-cased or the regex is narrowed), or (2) tighten `LANDED_STATUS_PATTERNS`/`OPEN_PR_STATUS_PATTERNS` (or the test's expected-offenders list) to account for the "amendment" narrative shape and re-verify against full history.
...
- **NEW this cycle — a latent test-suite gap, not an app regression.** See the **CRITICAL finding** section above in full: `check-docs-alignment.test.mjs`'s historical-ledger-replay test fails against a full git history (66 false positives on old `FEAT-112` rows) but is explicitly self-skipping on the shallow clones CI always uses, so **CI is unaffected and stays green**. This surfaced only because this cycle unshallowed the repo for accurate commit-count stats. Recommend a human decide whether to narrow the `LANDED_STATUS_PATTERNS`/`OPEN_PR_STATUS_PATTERNS` regexes to handle the "amendment PR, original already merged" narrative shape, or accept this as an expected local-only false-positive rate.
