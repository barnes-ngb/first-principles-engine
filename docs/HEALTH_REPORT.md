...
| `useQuestSession.ts` | 2,275 | KNOWN, flat. `CLAUDE.md`'s note reads 2,275L — exact match. |
...
- **`ReviewPage` (`src/features/review/ReviewPage.tsx`) and the standalone `/curriculum` route are undocumented in `CLAUDE.md`.** These are real, shipped features (referencing `UX-425`/`UX-426` in their own source comments) that predate this audit cycle but are absent from `CLAUDE.md`'s route/structure notes. Out of this audit's auto-fix scope (`CLAUDE.md` prose excluded by policy) — needs a deliberate doc pass, not a mechanical one.
- **`CurriculumTab.tsx` grew +737 lines this cycle (1,120→1,857L)** and is not in `CLAUDE.md`'s Known Technical Debt list at all, despite now being one of the five largest non-test source files in the repo and within ~150 lines of the 2,000L threshold the repo otherwise treats as a decomposition trigger.
