...
| **D9** (FEAT-49) | Review-chat infrastructure to reuse | (a) the `shellyChat` task + `useShellyChatActions` staging; (b) the `evaluationSessions` interactive-eval session pattern | **RECOMMEND a — reuse both, split by role** (see §11.6): the parent-facing Review Chat reuses the `shellyChat` propose→confirm→write + mid-chat-upload staging (it already exists, is confirm-gated, and handles `<action>` blocks), scoped by a new `foundationsReview` task; the **"Test it"** path routes into the `evaluationSessions`/Knowledge Mine quest pipeline (which already produces the *kid* evidence). Not either/or. |
