...
- **Slice 1 — Graph as data + deterministic bootstrap + read-only Foundations tab.** Ship the (owner-curated) graph module; a `buildLearnerModelDeterministic()` that reuses `deriveWorkingLevelMastery` to write `learnerModels/{childId}` (states + evidence trails only); the Foundations tab renders it and absorbs Learning Profile. **This is the smallest thing that produces a real stored model** — useful day one, no AI. *Revert: delete the tab + collection writer; nothing else depends on it.*
...
|---|---|---|---|
| **D1** | Where the per-child model is stored | (a) new `learnerModels/{childId}` collection; (b) a field on the child doc (like `dispositionCache`) | **(a)** — it's large and grows an evidence trail; a field bloats the hot child doc |
| **D2** | Graph versioning/storage | (a) versioned TS module in `src/core/foundations/`, PR-reviewed; (b) Firestore config doc editable without deploy | **(a)** — matches `curriculumMap`; keeps the spine reviewable/diff-able; no new write path |
| **D3** | Bootstrap band boundaries | Reuse phonics L1–8 / math L1–8 → K–5 mapping in the graph appendices as-is, or re-tune | Ship the appendix mapping as v0; **owner tunes** during curation |
| **D4** | Synthesis cadence & stale detection | (a) own weekly beat + timestamp-diff staleness; (b) piggyback `weeklyReview`; (c) explicit `learnerModelDirty` flag written at event points | **(a)** to start; revisit (b) once the beat is proven |
| **D5** | Deterministic/LLM split boundary | Exactly where judgment starts — e.g. does `confidence` derive mechanically from evidence count, or is it LLM-judged? | Mechanical where possible; LLM only for *conflict* resolution |
| **D6** | Cost ceiling | Per-child weekly + on-demand debounce window (e.g. ≥6h between on-demand regens) | Set a soft monthly ceiling mirroring `weeklyReview` monitoring |
| **D7** | Sight-word / mastery thresholds | Reuse FEAT-36's `SIGHT_WORD_MASTERED_THRESHOLD = 0.8` and gate-3 rule verbatim, or set model-specific ones | **Reuse verbatim** — do not fork tunables |
| **D8** | Confidence → ask routing | What confidence level triggers an `openQuestion`, and the default routed surface (quest vs eval) | Route thin/conflicting to a **quest** by default (kid-facing, low-friction) |
