...
export interface LearnerModel {
  ...
  // FIX-219 / UX-322: `status` is set by the seeder and then PROMOTED off
  // `no-data` by every writer that lands evidence, through the one shared rule
  // in `core/foundations/modelStatus.ts`. It had never been written by anything
  // but the seeder, so a child seeded before their first signal stayed `no-data`
  // forever — and five consumers read that as *no model at all*. It only ever
  // promotes: never demotes, never overwrites `synthesized`.
...
> **✅ SHIPPED 2026-07-25 (FEAT-66) — how it actually landed, and where it differs from the sketch above.**
>
> The Foundations tab is now a **write surface** for this one override, and only this one. Tapping a concept opens the same evidence drawer, which now carries a **"Record what you've seen"** section: three plain-language choices (`solid` / `forming` / `frontier`, labelled from the shared `STATE_PHRASE` — `src/features/foundations-review/statePhrase.ts`), an optional "What did you see?" note, and **propose → confirm → write** via the *existing* `ReviewActionConfirmCard`. Nothing writes on a tap.
>
> **Differences from the sketch, all deliberate:**
> - **No second write path, and no separate attestation map.** The override builds an ordinary `{ kind: 'attest' }` `FoundationsReviewAction` and goes through the **same** pure projector (`applyReviewActionToModel`) and the **same** merge-writer the Review Chat uses. That writer was extracted to `src/features/foundations-review/writeReviewAction.ts` so the merge shape (merge-only, **single-key `conceptStates`**, `synthesisStaleAt` stamped) has exactly one definition. Durability already comes from the re-seed guard + the FEAT-76 attestation freeze — a parallel override store would have been a second source of truth.
> - **`not-yet` is not offered.** `ReviewProposedState` excludes it by design: attesting *downward to nothing* is not a thing the vocabulary supports (`not-yet` = "we haven't seen it", an absence of evidence).
> - **"Revert to synthesized" is not offered.** There is nothing to revert *to*: a concept's derived read is not retained beside the attestation. The reconcile flow below is the real answer to "the model disagrees".
>
> **The reconcile affordance — FEAT-76's `needsReconcile` flag finally has a consumer.** FEAT-76 set `ConceptStateEntry.needsReconcile` when a guided eval disagreed with a parent attestation (state kept, `eval` evidence appended) and named this tab as the surface; until this run **no UI read it**, so a disagreement sat invisible in the data. Now: a quiet marker on the terrain chip (never a colour change — the attested state is still in force), and in the drawer a neutral notice — *"The model has a new take on this — view & reconcile."* — showing **both reads** (yours first, then the eval's, each with its date and scrubbed note). Two ways out, both ordinary confirm-gated attestations: **Keep my word** (re-attests the standing state) or **Take the model's read** (attests at the eval's state — recorded as the parent's deliberate choice, which is the honest record). Doing nothing changes nothing.
>
> Supporting mechanics:
> - **`EvidenceRef.readState`** — the state *that ref itself asserted*, as distinct from `ConceptStateEntry.state` (where the concept landed after every writer had its say). Stamped on both `eval` refs (`applyEvalFindingsToModel`) and `attestation` refs (`applyReviewActionToModel`). The eval side is what lets "take the model's read" offer a concrete state at all, since the frozen path keeps the parent's. The attestation side matters because a derived writer can move `entry.state` **while a disagreement is still standing** (a quest upgrade, a coverage claim): without it the reconcile view would report a state the parent never said, and "keep my word" would re-attest that drift. Readers fall back to `entry.state` on pre-FEAT-66 refs.
> - **Explicit `needsReconcile: false` on attest** — an absent key would leave the stored `true` in place under Firestore's deep merge. A `covered` write deliberately leaves a standing disagreement alone: only a parent resolves it.
> - **Capability gate, not a name gate.** `/progress` is **not** behind `RequireParent` (`router.tsx` — only the nav link is hidden), and a kid profile's own `activeChildId` is populated, so `childId` alone would have exposed the override to a child. Both the controls and the write handler gate on `useProfile().canEdit`, the same capability the route guard uses (ARCH-41/42/43 — capability, never a name). Kids can still read the terrain and the evidence trail; they cannot attest.
...
- **Slice 1 — Graph as data + deterministic bootstrap + read-only diag preview. ✅ SHIPPED (FEAT-48).** Shipped: the OWNER-CURATED graphs transcribed to a versioned TS module (`src/core/foundations/` — `types.ts`, `readingGraph.ts` [31 nodes], `mathGraph.ts` [29 nodes], `index.ts`, validation tests); `LearnerModel` types + the `learnerModels/{childId}` collection helper/converter; a pure `seedLearnerModel(graphs, childId, snapshot, skillMap, sightWordData)` that implements the graphs' own seeding sections (band-below-working-level → `solid`, at-level → `frontier`, above → `not-yet`; L7/L8 by node id; sight-word share vs the imported `SIGHT_WORD_MASTERED_THRESHOLD`; gate-3 priority skills + completed programs → `solid`; evidence-only strands → `not-yet`; invariant: every non-`not-yet` state carries ≥1 EvidenceRef; degrades on sparse data), plus `mergeSeededModel` preserving on re-seed **any entry carrying evidence the seeder cannot re-derive** (FIX-219 / UX-290 — it originally named `attestation` and `curriculumPosition` only, so a guided eval's read and a Knowledge Mine result were erasable by one button press; the rule is now stated against the four kinds the seeder itself emits, so the next `EvidenceKind` member is protected by default); and a **parent-only `?diag=1` seed/preview panel** on the Progress page (writes ONLY `learnerModels`, merge). **FIX-219 / UX-286 gave the model a second, non-diagnostic door**: the seed body moved into the shared `core/foundations/bootstrapLearnerModel.ts` and the **Foundations tab creates the model create-only** on first read of a child whose document is absent, capability-gated. Until then that `?diag=1` button was the *only* thing in the app that could create a learner model, and every other writer bailed on the document's absence — so on a family that never typed the flag the engine's central document did not exist. The button stays, as the deliberate re-seed. **This is the smallest thing that produces a real stored model** — useful day one, no AI. *Revert: delete `src/core/foundations/` + the collection writer + the diag panel; nothing else depends on it.*
...
- **Slice 3 — Foundations tab proper. ✅ SHIPPED (FEAT-65, Phase 3b — read-only).** `FoundationsTab` inserted at index 0 of `ProgressPage` (tabs converted to a `{ label, render }[]` descriptor array, §6.4); the standalone **Learning Profile tab is absorbed** — `DispositionProfile` embeds as the final section, unchanged. Renders the model as terrain (concept states grouped reading→math, four-state-colored tappable chips) + `whatMattersNext` focus + full moves + `modalityCalibration` in plain language + the `changeFeed` "What moved" + routed open questions. **Tap-a-concept → read-only evidence drawer** (`EvidenceRef[]` as plain-language source lines via `evidenceSourceLine`, never the seeded jargon note). §14 enforced by a shared `scrubDisplayJargon` + a render/presenter test (no band/level/percent/node-id leaks). Reads via the new shared `useLearnerModel` hook; **no writes, no `skillSnapshots` touch, no new synthesis.** **The §6.3 override/attest write path is DEFERRED** to a follow-up row (this run stays read-only per its scope); when built it reuses the existing `applyReviewActionToModel` `attest` path — not a second write path. *Revert: `/progress` restores the standalone Learning Profile tab.*
  - **FEAT-66 — the §6.3 override lands, and `needsReconcile` gets its consumer. ✅ SHIPPED (2026-07-25).** Closes the FEAT-65 deferral above **and** the FEAT-76 loose end. The evidence drawer gains a **"Record what you've seen"** section (three plain-language choices + optional note) and, on a concept a guided eval disagreed with, a neutral **"the model has a new take — view & reconcile"** notice showing **both reads** with two confirm-gated ways out (keep my word / take the model's read). Every route builds the same `{ kind: 'attest' }` action and goes through the **same** projector + the **extracted single writer** `writeReviewAction.ts` — `useFoundationsReview.applyAction` now calls it too, and its tests passed **unmodified**, which is the proof the extraction was behaviour-preserving. `learnerModels`-only, merge-only, single-key `conceptStates`; no `skillSnapshots` / XP / hours / rules touch; nothing writes on a single tap. Supporting: `EvidenceRef.readState` (what an eval read, since the frozen path keeps the parent's state) + an explicit `needsReconcile: false` on attest (Firestore's deep merge would keep an absent key's stored `true`). Details in **§6.3**. *Revert: drop the drawer's override + reconcile sections; the shared writer collapses back into the hook.*
...
>
> **✅ BUILT (FEAT-70, 2026-07-26) — the review note's "was anything tricky?" is the FOURTH daily struggle signal, and a different MOMENT.** FEAT-68/69 wired the in-the-moment signals; grade notes were split out because `ChecklistItem.gradeResult` is **free text with no struggle flag and no parse** — nothing deterministic to resolve. FEAT-70 closes it with **structured capture, not an LLM parse**, and the rejection of the parse option is the load-bearing decision: (1) **no-guess discipline** — every existing path into the queue (`resolveNativePosition`, `conceptsForTags`) returns `[]` rather than invent a mapping, and an LLM mapping prose → concept ids is precisely the fabrication surface those were written to avoid; it would be the only non-deterministic seeder in the queue; (2) **the note is deliberately no-judge** — its placeholder was softened on purpose (`// no-judge v1`), and silently running a judgment parse over that text would re-introduce, invisibly, the judgment the rewrite removed; (3) **testability** — a fire-and-forget model call with no visible result cannot be asserted, one boolean can. So the signal is an explicit, optional, default-off parent toggle (**"Was anything here tricky?"**) inside the existing Quick Review editor, persisting a new additive `ChecklistItem.reviewFlaggedTricky?: boolean` **in the same `persistDayLogImmediate` write** as the note. **Why it is not a duplicate of the chip:** Quick Review is gated on `item.evidenceArtifactId`, so it appears only *after* work was captured and the parent looked at what came back — often when the in-the-moment chip already reads `got-it`. A miss noticed at review time is information the model never heard before. It stamps its own `GRADE_NOTE_RETEST_REASON` ("Flagged as tricky when reviewing today's work") so the parent-facing ask reads honestly about which signal seeded it. **Fires only on the false→true transition** (`shouldSeedFromReviewNote`, pure + exported): `withOpenQuestion` dedups the `openQuestion`, but the `queueTest` branch appends a `changeFeed` line **unconditionally**, so re-saving a note on an already-flagged item would stack duplicate change lines onto the model — the guard is **required**, not belt-and-braces. **Nothing downstream moved:** `enqueueStuckRetests`, `resolveStuckConcepts`, `resolveTagConcepts`, `TAG_CONCEPT_BRIDGE` and the workbook bridge are **untouched** — this is a fourth *caller* of a tested path, not a fourth path. **No `skillSnapshots` write** from this path (the parallel `conceptualBlocks` write stays exclusive to the mastery chip), and the toggle never sets `mastery` or `engagement` — three fields, three meanings. **Coverage caveat (state it plainly):** this does **not** close the grade-note loop everywhere. It lights up only when the item captured evidence, the note has not already been saved (`!item.gradeResult` gates the affordance), the parent flips the toggle, **and** the item resolves through a curated workbook bridge or a mapped `skillTag` — otherwise `[]`, no write, no guess. Coverage still grows only by owner curation of the bridge table (FEAT-63/69 discipline), never as a side effect of a wiring run. Teach-backs (**FEAT-71**) remain deferred — still a done-flag + coarse subject only, with no conceptId.
...
- **Phase 3b — the Foundations tab (slice 3) + the one-line planner surface (slice 7). ✅ SHIPPED (FEAT-65, read-only).** 3a fed the model's judgment layer and wired the ambient consumers; 3b gives it a parent-facing home (`FoundationsTab` — terrain map + read-only evidence drawer + `whatMattersNext`/`narrative`/`openQuestions`/`changeFeed` sections, dispositions absorbed as a section) and the *"This week's foundation focus: {X}, because {Y}"* line on Plan My Week (`FoundationsFocusLine`, from `synthesis.whatMattersNext[0]`), plus a deterministic `→ solid` loop-confirmation card (closes the design-review G3). Shared read via `useLearnerModel`. **Deferred to follow-up rows:** ~~the §6.3 attest-override write path (this run is read-only)~~ — **closed by FEAT-66 (2026-07-25)**, which also gave FEAT-76's `needsReconcile` flag its first consumer (see §6.3); a `ConceptStateKind → SkillStatus` adapter to reuse the learning-map cards instead of the diag-style render (**FEAT-67, still open**).
