...
>
> Two supporting mechanics were needed: `EvidenceRef.readState` (FEAT-66, stamped by `applyEvalFindingsToModel` on every `eval` ref) records what an eval read, since on the frozen path the stored state stays the parent's — without it "take the model's read" has no state to offer, and a pre-FEAT-66 ref simply doesn't get that button. And the `attest` branch now clears `needsReconcile` with an **explicit `false`** (an absent key would leave the stored `true` in place under Firestore's deep merge); a `covered` write deliberately leaves a standing disagreement alone — only a parent resolves it.
