...
* Print — PDF via jsPDF, settings dialog (Letter/Half-letter/A4/Booklet, background options, sight word style), images via Firebase SDK getBlob, aspect-ratio-locked (3:2) image container matching editor + reader exactly (percentage-based sticker positions map 1:1), cover-fit for scene images, contain-fit for stickers, dynamic font sizing with overflow guard
