...
* Book Editor — page editor with text, voice recording, speech-to-text dictation, AI scene generation ("Make a Scene"), sticker picker, drag-to-position images, text sizing (Big/Medium/Small), font family (Handwriting/Print/Pixel), page reorder, Together Book toggle, "Finish My Book" with cover picker. Combined drawing flow: single "Add My Drawing" button opens camera, then choice dialog (use as-is, clean up, reimagine with intensity slider, make a sticker, make a scene). Reimagine processes in background — dialog closes immediately, small "Reimagining..." chip shown in corner, success notification with thumbnail appears when done, tap opens choice dialog (add to page / make sticker / save to gallery / discard). Auto-saves to sticker library after 5 min inactivity. Client-side 3 min timeout. Blocking spinners only used for < 5 second operations (cleanup, as-is). Sticker toolbar dismisses on outside tap (text area, action buttons). Background edit icon sits outside image area (small wallpaper icon → menu with Change/Remove). Title bar split into two rows: Row 1 (back + editable title with tinted bg + save indicator), Row 2 (Read/Print/Finish as scrollable chips)
