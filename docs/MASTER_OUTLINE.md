...
- **Quick Capture (note, photo, audio)** — with optional duration field; when filled, writes to compliance hours via `source: 'quick-capture'`. Empty preserves current artifact-only behavior. Helper text updates live as Shelly types; saves cleanly even if hours write fails (capture is primary value, hours is a bonus).
...
15. **Quick Capture is the dominant "log something extra" affordance** — Creative Time and Extra Activity are alternative entry points pending consolidation review (Path B, deferred). Quick Capture now carries an optional duration so a photo/audio/note can close the compliance-hours loop without forcing Shelly to open a second accordion.
