...
* Sticker Generation — gpt-image-1 transparent backgrounds, post-generation tagging (tags + Lincoln/London/Both profile), Sticker Library in Settings, hardened Claude Haiku rewriter with explicit character-to-visual-description examples (handles Mario/Pikachu/Elsa/etc.), regex fallback strip when rewriter unavailable, friendly error messages on content policy blocks
