Barnes Family Homeschool — Master Project Outline v6 Version: March 21, 2026 Status: Living document — updated after Avatar/Armor system, Book Polish, Story Guide, Evaluation Pattern Detection Project Summary Homeschool management app for the Barnes family: Shelly (parent, fibromyalgia), Nathan (dad, builds the system), Lincoln (10, neurodivergent, speech challenges), London (6, loves drawing/stories). Both boys. Tech: React + TypeScript + Vite, Firebase (Auth, Firestore, Storage, Cloud Functions, Hosting), MUI, Anthropic Claude API (Sonnet for planning/evaluation/stories, Haiku for prompt rewriting), OpenAI DALL-E 3 (scenes + armor sheets) + gpt-image-1 (transparent stickers + photo transform). Repo: github.com/barnes-ngb/first-principles-engine Live: first-principles-engine.web.app Scale: 52k+ lines TypeScript, 30+ test files, 600+ tests, 32+ Firestore collections, 0 TS errors What's Built and Working Navigation Parent: Today, Plan My Week, Weekly Review, Progress, Records, Dad Lab, Settings Kid: Today, Knowledge Mine, My Armor, My Books, My Stuff, Dad Lab Today Page
...
* Sticker Library tab — all generated stickers, tag editing, child profile assignment Cloud Functions (5 deployed)
1. chat — plan/chat/evaluate/quest/generateStory/analyzePatterns (taskTypes)
2. weeklyReview — scheduled Sunday 7pm CT
3. generateActivity — lesson card generation
4. generateImage — DALL-E 3 (scenes + armor sheets) + gpt-image-1 (stickers + photo transform) + Haiku rewriter; taskTypes: scene, sticker, generateArmorSheet, transformAvatarPhoto, generateBaseCharacter
5. healthCheck — diagnostic AI Context Pipeline System prompt includes: charter preamble, enriched child context (skill snapshot, workbook configs, recent sessions), evaluation findings (frontier, recommendations, skip list), engagement data (last 14 days), grade results, week focus, quest prompt, story generation prompt Firestore Collections (32+) families/{familyId}/ + children, weeks, days, artifacts, hours, hoursAdjustments, skillSnapshots, workbookConfigs, plannerConversations, lessonCards, avatarProfiles, dailyPlans, weeklyReviews, aiUsage, evaluationSessions, ladders, ladderProgress, milestoneProgress, sessions, projects, labSessions, weeklyScores, dadLabReports, books, bookPages, sightWordProgress, xpLedger, xpEventLog, readingSessions, dailyArmorSessions, stickerLibrary, weeklyScores What's Built but Untested with Real Users
