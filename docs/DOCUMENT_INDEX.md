...
| `foundations/FAST_PHONICS_BRIDGE_V0.md` | **DRAFT — pending owner curation** (2026-07-04, FEAT-49) | External-curriculum bridge draft — maps Fast Phonics (Reading Eggs) **Peaks 1–20** to reading-graph node ids, so completing a peak supplies `covered` evidence for its concepts. Includes the versioned bridge-data shape (`CurriculumBridge` / `BridgeUnit`), the covered≠mastered semantics (caps at `forming`, quiz scores promote only *within* forming), the sight-word multi-source rule, a worked "Peak 13 complete" example, and a **scope-&-sequence gaps** section flagging uncertain per-peak grapheme boundaries for owner verification against the account. **Not authoritative yet** — same curation workflow as the graphs. Appendix to `LEARNER_MODEL_DESIGN.md` §12. |
