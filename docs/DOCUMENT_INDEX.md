...
| `foundations/MATHSEEDS_BRIDGE_V0.md` | **DRAFT — PENDING OWNER CURATION** (2026-07-15, FEAT-63; not shipped as data) | External-curriculum bridge **draft** — maps Mathseeds levels to math-graph node ids so a curated version can turn the tracked Mathseeds position (config reads Level 122) into `covered` evidence, closing the model's blind spot on the child's primary **math** curriculum. Reconstructed from general program knowledge, **not** an official scope & sequence; every level-band row carries a confidence flag and maps onto CURATED math-graph nodes only. Inherits the Fast Phonics semantics (covered≠mastered `forming` cap, cumulative, never-downgrade, dedup). Curation-question sections cover the lesson-number meaning (the `lessonToUnit` §0.2 slot), per-concept level bands, and where Level 122 sits. FEAT-63 shipped the *wiring* (`workbookBridge.ts`); this is the data half awaiting curation (FEAT-46→47 pattern). Appendix to `LEARNER_MODEL_DESIGN.md` §12. |
| `foundations/TGTB_LA1_BRIDGE_V0.md` | **DRAFT — PENDING OWNER CURATION** (2026-07-15, FEAT-63; not shipped as data) | External-curriculum bridge **draft** — maps The Good and the Beautiful **Language Arts Level 1** lessons to reading-graph node ids so a curated version can turn the tracked TGTB LA1 position (config reads Level 110) into `covered` evidence, **multi-source** alongside Fast Phonics on shared reading nodes. Reconstructed from general course knowledge, **not** an official scope & sequence; every lesson-band row carries a confidence flag and maps onto CURATED reading-graph nodes only. Inherits the Fast Phonics semantics. Curation-question sections cover the lesson-number meaning (the `lessonToUnit` §0.2 slot), per-concept lesson ranges, TGTB's own phonics order, and where Level 110 sits. FEAT-63 shipped the wiring; this is the data half awaiting curation. Appendix to `LEARNER_MODEL_DESIGN.md` §12. |
