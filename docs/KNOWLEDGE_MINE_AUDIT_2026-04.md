...
| P1-3 | ✅ FIXED — Comprehension prompt restructured | A | ~~Only 3 difficulty bands (2 levels each) with vague guidance. Phonics has 6 specific bands with concrete question types. Comprehension quality is unpredictable.~~ **FIXED (Apr 20, 2026):** `buildComprehensionQuestPrompt` rewritten to mirror phonics structure — 6 concrete question types per 2-level band (18 total across L1-2, L3-4, L5-6), Minecraft-themed passages required, passage-length rules per level, auto-read TTS note, kid-friendly language ban list (no "comprehension"/"inference"/"main idea"/"author's purpose"/etc.), rotation rule, expanded skill-tag vocabulary. See `functions/src/ai/chat.ts:770`. | M |
