...
> **Status (FEAT-172, 2026-09-02).** Two corrections to the FEAT-169 wiring, from Shelly's next report. **(1) A list the parent typed wins.** `storyPracticeWords.parseRequestedWords` reads an explicit list out of the story idea (*"include these sight words: our, friend, …"* — a `words` cue with a `:` / dash / "like" / "are", then comma-separated single words, ending at the sentence) and `resolveStoryWords` makes it the story's word list; the practice list is the fallback only when the parent named none, and a story with neither carries no list. The confirmation lines name the source in play — *"the words you asked for"* vs *"some of {child}'s practice words"* — before the tap and in the draft turn, so the two are never confused; the after-line is still checked against the page text for both. **(2) The book binds to the child it is FOR, never to the header.** `BookGenerateChat` gives a parent a *"This story is for"* picker (defaulting to the active child, locked once a story exists or when resuming), a resumed draft hydrates its child from the book (`resumeForChildId`, since a parent's shelf holds every child's books), and that one `childId` drives the practice-word read, the server's age / interests / word-mastery context and the shelf the draft lands on. A kid profile is always itself. Nothing infers a child from prose. Path C is still untouched.

> **Status (FEAT-173, 2026-09-02) — correction (2) above is reverted.** The owner rejected FEAT-172's child-binding diagnosis: *"Shelly should have been in London's profile."* **The active profile is the context**, as on every other surface, and a second child-selector inside the book chat is exactly the duplicate surface this project has been removing. FEAT-173 removed the "This story is for" picker, `resumeForChildId`, and the per-persist `childId` / `createdFor` rewrite (a draft is not re-homed by a persist); the chat's reads and writes follow the header's active child again. Correction (1) — a typed list wins — stands unchanged. `useSightWordProgress`'s re-arm on a child change is kept: it is a read-path correctness fix for a header switch, not a picker.
...
> **Status (FEAT-97 → FEAT-172).** The fixed 6144 became `maxTokensForPageCount(pageCount)` in FEAT-97 (2048 + 512/page: 6 / 10 / 14 pages → 5120 / 7168 / 9216). On the Sonnet-5 generation `generateStory` runs adaptive thinking at the default HIGH effort with thinking tokens counted against `max_tokens`, and FEAT-169's diagnostic **confirmed** that 7168 still cut a Normal / 10-page book with a word list short (`stop_reason=max_tokens`). FEAT-172 resized it on that evidence: base **8192** (the thinking headroom — the visible story is only ~100 tokens a page) + **512/page** + **64/word** (`storyPageBudget.ts`), clamped to the unchanged **[4096, 16384]** rails → 6 / 10 / 14 pages = **11264 / 13312 / 15360**, Shelly's 10 pages + 12 words = **14080**, 14 pages + 15 practice words = 16320, and a 22-word list on 14 pages clamps at the ceiling. `max_tokens` is a ceiling, not a spend; the real bound is the 300 s callable / CF window, which 16k output tokens fits with margin. `reviseStory` shares the function and gains the same headroom.
