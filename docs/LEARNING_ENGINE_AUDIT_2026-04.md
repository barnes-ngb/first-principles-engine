...
| # | Gap | Severity | Notes |
|---|---|---|---|
| G20 | ~~`workingLevels` not displayed in Skill Snapshot UI~~ | ~~Medium~~ | **FIXED** (B.2). Added Working Levels section to Progress → Skill Snapshot tab (`WorkingLevelsSection.tsx`). Displays current level, source badge, last-updated, and evidence for phonics/comprehension/math. Parents can adjust manually (±1 controls with optional note → writes `source: 'manual'`), and revert manual overrides. Respects `QUEST_MODE_LEVEL_CAP` ceilings. |
...
| `handleSaveAndApply` (EvaluateChatPage.tsx:592) | `conceptualBlocks` + `blocksUpdatedAt` (from pattern analysis, **merged via `mergeBlock` + `updateDoc`** since Phase 1 — see footnote) | `skillSnapshots/{childId}.conceptualBlocks` | AI `skillSnapshot` slice — ADDRESS_NOW + RESOLVING + DEFER sections (contextSlices.ts `formatConceptualBlocks`) | — |

> **Footnote (added 2026-05-16, post-Phase-1 verification):** This row originally read "overwritten not merged." Phase 1 of the evaluation methodology shipped 2026-04-21 (`docs/EVALUATION_METHODOLOGY_2026-04.md` §3, MASTER_OUTLINE.md:181) and replaced the wholesale-overwrite with `mergeBlock` (`src/core/utils/blockerLifecycle.ts`) + `updateDoc`. All four conceptualBlocks writers (evaluation / quest / scan / parent) now share the same merge path — cross-writer wipes are no longer possible. The AI injection also widened: `formatConceptualBlocks` now emits ADDRESS_NOW, RESOLVING, and DEFER sections (RESOLVED kept in data but omitted from prompts), not ADDRESS_NOW only.
