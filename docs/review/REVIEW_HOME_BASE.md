...
| **ARCH-09** | 1 | **IN PROGRESS** | Build Step 1 continued (decompose PR open): the page's effects + flow handlers (send/response, image gen/refine, image analysis/attach upload cluster, thread CRUD) moved to **`useShellyChatFlows`**, and the branchy reflection-suggestion heuristics extracted to the pure **`reflectionSuggestions`** module. The page is now a thin shell composing `useShellyChatState` + `useShellyChatFlows`. Behavior-preserving — same Firestore reads/writes (`shellyChatThreads`/`shellyChatMessages` only), same props/route contract, CF request/response shape unchanged; **no `applyChatAction`/write capability added** (that's Build Step 3). ShellyChatPage **1,632→611L**. Reserved `useShellyChatActions` insertion point still documented in `sendToAI`. Earlier step: 23+ `useState`/`useRef` → `useShellyChatState`; `[FOLLOWUP]` parser → `parseFollowups` ([PR #1274](https://github.com/barnes-ngb/first-principles-engine/pull/1274)). Seams per `docs/SHELLY_PORTAL_CONTEXT.md §6`. | branch `claude/eloquent-meitner-Md0Vm` — `src/features/shelly-chat/{ShellyChatPage,useShellyChatState,useShellyChatFlows,reflectionSuggestions}.tsx/ts` |
...
| **TEST-01** | 1 | **PARTIALLY ADDRESSED** | `shelly-chat` coverage extended with the decompose PR: added `reflectionSuggestions.test.ts` (10 — frustration / late-week dropoff / high-engagement branches + the 3-cap) and `ShellyChatPage.test.tsx` (3 — first render/shell smoke test). Now **5 files / 35 tests** (was 3 / 22): + `parseFollowups.test.ts` (10), `formatRelativeTime.test.ts` (6), `useShellyChatState.test.ts` (6). `progress`, `dad-lab` still at 0. The originally-scoped `parseChatActions`/`useShellyChatActions` tests wait on Build Step 3 (the write layer they cover isn't built yet). (Note: the existing `shellyChat.test.ts` is **server-side**, `functions/src/ai/tasks/`.) | confirmed in `ARCHITECTURE_AUDIT_2026-05.md §1.3` |
...
| **FEAT-01** | 2 | OPEN (new) | Shelly Chat control portal — see `barnes-shelly-chat-portal-design.md`. Ask AI becomes Shelly's read+assist+write portal via the proven AI-proposes → human-confirms → one-writer-commits loop (no tool-use). Tiers A (read: add `childSkillMaps` slice) + B (sight words, soft profile fields) ready; Tier C (snapshot writes) after FUNC-02. Closes ARCH-09 + TEST-01 + ETHOS-01 along the way; pairs with ARCH-10. Phase-1 build prompt in the design doc §9. | `src/features/shelly-chat/`, `functions/src/ai/tasks/shellyChat.ts`, `functions/src/ai/contextSlices.ts`; grounded in `docs/SHELLY_PORTAL_CONTEXT.md` + `docs/SHELLY_PORTAL_FEEDBACK_LOOP.md` |
