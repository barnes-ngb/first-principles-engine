...
| **FEAT-04** | 2 | OPEN | **Encoding (spell-it) check flow.** Add an encode/dictation check ("spell the word you just read") alongside the existing decode checks — encoding is a distinct, harder skill that reveals gaps decoding hides. **Not started.** Every assessment mode is decode/recognition: quest modes are `Phonics`/`Comprehension`/`Fluency`/`Math` (`questTypes.ts:12-18`), all selection- or read-aloud-based (`chat.ts` quest prompt builders); guided eval has the parent observe the child *read* (`EvaluateChatPage.tsx`); sight-word mastery = repeated recognition (`sightWordMastery.ts`). No quest/eval mode or finding status represents "spelled it / couldn't spell it." (Note: `RoutineItemKey.Spelling`/`SpellingDictation` are parent-tracked daily *checklist* items, not an interactive assessment.) **Depends on** extending the quest mode discriminator + the `wordProgress` store (encoding stat). **Owner:** dedicated build chat (net-new mode + prompt + finding/score shape). | `src/features/quest/questTypes.ts:12-18`, `functions/src/ai/chat.ts` (quest prompt builders), `src/features/evaluate/EvaluateChatPage.tsx`, `src/features/books/sightWordMastery.ts` |
...
| **FEAT-06** | 2 | OPEN | **Math evaluation parity.** A reading-style guided-evaluation + working-level derivation for the math domain. **Partial:** the reading domain has full eval→snapshot→working-level; **math working-level derivation from eval findings is a TODO** (`EvaluateChatPage.tsx:540`) and `FINDINGS_PIPELINE.md` lists it as NOT done (math scan-derived levels work, but eval-derived do not). MASTER_OUTLINE backlog #6. **Owner:** dedicated build chat (mirrors the reading eval flow). | `src/features/evaluate/EvaluateChatPage.tsx:540`; `docs/FINDINGS_PIPELINE.md` ("What This Pipeline Does NOT Do"); MASTER_OUTLINE #6 |
| **FEAT-07** | 2 | OPEN | **Eval close-the-loop automation.** Close the evaluate loop: reading a practice story produces new findings, findings re-test on a schedule, and re-eval triggers from engagement patterns. **Not started** — `FINDINGS_PIPELINE.md` ("What This Pipeline Does NOT Do" + Known Limitations 2-4) confirms the practice-story loop is human-in-the-loop, reading a story yields no findings (loop doesn't close), findings never expire/re-test, and there's no auto story-gen. MASTER_OUTLINE backlog #5. **Owner:** dedicated build chat (design-first; touches the findings pipeline). | `docs/FINDINGS_PIPELINE.md` (Known Limitations 2-4); MASTER_OUTLINE #5 |
