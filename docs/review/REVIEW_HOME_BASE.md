...
| **ARCH-09** | 1 | **IN PROGRESS** | Decompose of `ShellyChatPage` proceeding in steps. **Step 1:** 23+ `useState`/`useRef` → `useShellyChatState`; `[FOLLOWUP]` parser → `parseFollowups` ([PR #1274](https://github.com/barnes-ngb/first-principles-engine/pull/1274)). **Decompose continued (this PR):** effects + flow handlers (send/response, image gen/refine, image analysis/attach upload cluster, thread CRUD) → **`useShellyChatFlows`**, and the branchy reflection-suggestion heuristics → pure **`reflectionSuggestions`**; the page is now a thin shell composing state + flows, **1,632→611L**, behavior-preserving (same `shellyChatThreads`/`shellyChatMessages` reads/writes, same props/route + CF shape, **no write capability added**). **Step 3a landed (no writes):** pure `parseChatActions` + `ChatAction` allowlist union + shared `src/core/utils/sanitizeJson` client port — all unwired (page untouched). The `useShellyChatActions` write layer + system-prompt grammar remain for 3b; reserved insertion point documented in `useShellyChatFlows` `sendToAI`. Seams per `docs/SHELLY_PORTAL_CONTEXT.md §6`. | [PR #1274](https://github.com/barnes-ngb/first-principles-engine/pull/1274) + this decompose PR — `src/features/shelly-chat/{ShellyChatPage,useShellyChatState,useShellyChatFlows,reflectionSuggestions,parseFollowups,parseChatActions}` + `src/core/utils/sanitizeJson.ts` |
...
| **TEST-01** | 1 | **PARTIALLY ADDRESSED** | `shelly-chat` well past 0. Step 1: `parseFollowups.test.ts` (10), `formatRelativeTime.test.ts` (6), `useShellyChatState.test.ts` (6). Step 3a: `parseChatActions.test.ts` (13) + shared `src/core/utils/sanitizeJson.test.ts` (11, port of the functions suite). Decompose PR: `reflectionSuggestions.test.ts` (10 — frustration / late-week dropoff / high-engagement branches + the 3-cap) and `ShellyChatPage.test.tsx` (3 — first render/shell smoke test). shelly-chat now **6 files / 48 tests** (plus the shared sanitizeJson port). `progress`, `dad-lab` still at 0. The `useShellyChatActions` write-layer tests still wait on Build Step 3b. (Note: the existing `shellyChat.test.ts` is **server-side**, `functions/src/ai/tasks/`.) | confirmed in `ARCHITECTURE_AUDIT_2026-05.md §1.3` |
