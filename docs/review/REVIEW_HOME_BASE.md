...
| **ARCH-09** | 1 | OPEN (new) | `ShellyChatPage.tsx` 1,653L — 23+ useState, image generation, thread management. Decomposition seams (`useShellyChatState` vs new `useShellyChatActions`) mapped in `docs/SHELLY_PORTAL_CONTEXT.md §6`. | `src/features/shelly-chat/ShellyChatPage.tsx` — defer until usage stabilizes |
...
| **TEST-01** | 1 | OPEN | `shelly-chat`, `progress`, `dad-lab` have **0** test files. Propose `shellyChat.logic.test.ts` + `DispositionProfile.test.tsx` next. First shelly-chat tests (`parseChatActions`, `useShellyChatActions` logic) scoped in `docs/SHELLY_PORTAL_CONTEXT.md §6`. (Note: the existing `shellyChat.test.ts` is **server-side**, `functions/src/ai/tasks/`.) | confirmed in `ARCHITECTURE_AUDIT_2026-05.md §1.3` |
