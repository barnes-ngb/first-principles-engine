...
| **DATA-03** | 4† | OPEN (investigated 2026-05-31) | **Confirmed: the repo configures NO automated backup.** Full sweep (root, `functions/`, `firebase.json`, `.firebaserc`, `scripts/`, `.github/workflows/`) found only **manual** `gcloud firestore export`/`import` *docs* in `08_RUNBOOK.md:89-101` — documentation, nothing scheduled. The two scheduled CFs (`weeklyReview` `evaluate.ts:1015`, `generateMonthlyReview` `monthlyReview.ts:96`) generate AI reviews, **not** backups. No backup CF, no Pub/Sub export schedule, no GCS bucket export, no PITR config in repo. App is Lincoln's legal MO record — **no in-repo recovery path** if data is lost/corrupted. **Recommendation (Option 1, zero code):** enable **Firestore PITR** (7-day recovery) + **scheduled backups** (daily, e.g. 14-day retention) in the GCP/Firebase console — managed by Google, right answer for a single-family app. Console: Firestore → Backups → *Create schedule* (or `gcloud firestore databases update --database='(default)' --enable-pitr` and `gcloud firestore backups schedules create --database='(default)' --recurrence=daily --retention=14d`). **⚠ Console-confirm (do from phone): is PITR on? Are scheduled backups configured?** — can't see live GCP from repo. Compliance **export** path (separate from backup) verified working: `RecordsPage.tsx` `handleExportHoursCsv`/`handleExportPortfolioMd`/daily-log/eval-MD/zip handlers intact (export = printable copy for the state; backup = survive a data disaster). **Promoted top-of-queue (highest-stakes).** | repo-wide; only `08_RUNBOOK.md:89-101` (manual docs). Investigated via `prompts/PROMPT_BACKUP_CHECK.md` |
