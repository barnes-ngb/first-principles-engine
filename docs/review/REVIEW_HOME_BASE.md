...
| **FEAT-02** | 2 | OPEN | **London kid-surface parity.** Bring London's (6) kid experience to parity with Lincoln's, or to a deliberate age-appropriate equivalent. **Not started.** Gating is hardcoded by child *name*, not an age/config flag: `useChildGating.ts` (`isLincoln` derive) drives `hideMine={!isLincoln}` so London's Hero Hub launcher drops the **Knowledge Mine** tile (`MyAvatarPage.tsx:197,1624`; `HeroLauncherTiles.tsx:25-27`; test `HeroLauncherTiles.test.tsx:40-46`). My Books + Workshop + Dad Lab are at parity; Kid Today differs only by cosmetic theme/greeting (`KidTodayView.tsx:102`). No London-specific quest/eval tuning exists — the outline's "London-specific evaluation flow" + "Knowledge Mine London tuning pending" are unimplemented. Gate is soft (the `/quest` route nav item is ungated). Overlaps FEAT-06 (math) for the eval side. **Owner:** the launcher-tile/eval-flow consistency fix is a small home-base run; the age-adjusted London evaluation/quest flow is dedicated-build-chat-sized. | `src/features/avatar/MyAvatarPage.tsx:197,1624`, `src/features/avatar/HeroLauncherTiles.tsx:25-27`, `src/features/today/KidTodayView.tsx:102`; MASTER_OUTLINE "London-specific evaluation flow" (#7) |
