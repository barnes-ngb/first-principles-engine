...
| **ARCH-09** | 1 | **IN PROGRESS** | Build Step 1 done: the 23+ `useState`/`useRef` extracted to `useShellyChatState` (typed, unit-tested); pure `[FOLLOWUP]` parser → `parseFollowups`; reserved `useShellyChatActions` insertion point documented in `sendToAI` (write layer = Build Step 3). Behavior-preserving — no caller-visible contract change; CF request/response shape unchanged. ShellyChatPage 1,653→1,632L. **"Defer until usage stabilizes" condition now met by decision:** the portal is about to add an actions layer to this page, so the decompose is a deliberate prerequisite, not premature. Seams per `docs/SHELLY_PORTAL_CONTEXT.md §6`. **Build Step 3a landed (portal extraction plumbing, no writes):** pure `parseChatActions` + `ChatAction` allowlist union + shared `src/core/utils/sanitizeJson` client port — all unwired (page untouched), zero behavior change; the write layer (`useShellyChatActions`) + system-prompt grammar remain for 3b. | [PR #1274](https://github.com/barnes-ngb/first-principles-engine/pull/1274) — `src/features/shelly-chat/{ShellyChatPage,useShellyChatState,parseFollowups}.tsx/ts`; 3a in `src/features/shelly-chat/parseChatActions.ts` + `src/core/utils/sanitizeJson.ts` |
...
| **TEST-01** | 1 | OPEN | `shelly-chat` **no longer at 0** — first client tests landed with ARCH-09 Build Step 1: `parseFollowups.test.ts` (10), `formatRelativeTime.test.ts` (6), `useShellyChatState.test.ts` (6) = 3 files / 22 tests. `progress`, `dad-lab` still at 0. Build Step 3a added `parseChatActions.test.ts` (13) + `src/core/utils/sanitizeJson.test.ts` (11, port of the functions suite) = the pure extractor is now fully covered; the `useShellyChatActions` write-layer tests still wait on Build Step 3b. (Note: the existing `shellyChat.test.ts` is **server-side**, `functions/src/ai/tasks/`.) | confirmed in `ARCHITECTURE_AUDIT_2026-05.md §1.3` |
