...
| **FUNC-02** | 2 | OPEN (green-lit) | Learning Map → Skill Snapshot write-through missing. Scan advances curriculum node but does NOT update conceptual blocks. Implements the FUNC-01 seam; build prompt ready in the decision doc. Write-through site already stubbed. | `useSkillMapWrite.ts:22` (stub) → `skillSnapshotWrites.ts` (`applyScanToSnapshot`) |
