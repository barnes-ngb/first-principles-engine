...
- **Numbers in prose are derived, never counted by hand.** If this fix's deliverable includes a document
  ...
  commit SHA, date) and mark it hand-counted. The case this exists for is a **survey** — a count read across a body of code larger than
  the change; a count over the run's own diff is pinned by that diff, so naming the command that prints it
  is derivation enough, and a short list is best **named rather than counted**.
...
**The verification suite runs on the exact tree you push, after the last edit.** A green run from before your
final fix is evidence about a tree that no longer exists. If you amend or add a commit after verifying, verify
again — and note that a moved head also invalidates a pending review ask, which must then be re-made against
the new commit.
