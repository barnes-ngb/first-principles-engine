...
| **Concept frontier / synthesis** — the synthesized "what matters next" read across a child's foundations concepts | `learnerModels/{childId}` | `evalModelWriteback.ts`, `questModelSync.ts`, `useFoundationsBootstrap.ts` (create-only), `writeReviewAction.ts`, `strandSessionWrites.ts`, `activityConfigWrites.ts`, `workbookPositionSync.ts`, `bootstrapLearnerModel.ts` (create-only) | Foundations tab, planner focus line (`FoundationsFocusLine`), Shelly chat, weeklyReview (via `synthesizeIfStale`) | **Added 2026-09-20 (`DOC-26`), filed by `ARCHITECTURE_AUDIT_2026-09-13.md` §2.1.** A store the 2026-05-30 ruling did not name because the Learner Model was still a `?diag=1`-only diagnostic when this table was written; `FIX-219`/`AUDIT-217` (2026-09) made it real for ordinary families. It is a **second, independent answer to "what does Lincoln know,"** running alongside `skillSnapshots` — not a replacement for that row's "what to teach next" authority, since a guided eval or quest reads this store's synthesized frontier to decide what's next while `skillSnapshots` is what a scan/eval/manual edit records as achieved. The two can disagree, and the codebase already designs for it: `core/foundations/evalModelSync.ts` flags a concept `needsReconcile` when a guided eval disagrees with a parent's word, resolved via the Foundations tab's confirm/override flow (`progress/conceptOverride.ts` — keep-my-word / take-the-model's-read). |
