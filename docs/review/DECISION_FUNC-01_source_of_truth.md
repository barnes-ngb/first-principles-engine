...
| **Concept frontier / synthesis** — the synthesized "what matters next" read across a child's foundations concepts | `learnerModels/{childId}` | `evalModelWriteback.ts`, `questModelSync.ts`, `useFoundationsBootstrap.ts` → `bootstrapLearnerModel.ts` (create-only), `writeReviewAction.ts`, `activityConfigWrites.ts` → `workbookPositionSync.ts`, `stuckRetestQueue.ts`, `learnerSynthesis.ts` (merge-writes the `synthesis` field) | Foundations tab, planner focus line (`FoundationsFocusLine`), Shelly chat, weeklyReview (via `synthesizeIfStale`) | **Added 2026-09-20 (`DOC-26`), filed by `ARCHITECTURE_AUDIT_2026-09-13.md` §2.1; writer list corrected on Codex review of the same PR** — the first draft named `strandSessionWrites.ts`, whose own header states *"No XP, no skillSnapshots, no learnerModels, no day-log write"*; it never touches this store and is removed. `learnerSynthesis.ts` and `stuckRetestQueue.ts` were confirmed as real writers (`db.doc(...learnerModels/${childId})` merge-writes) and added. A store the 2026-05-30 ruling did not name because the Learner Model was still a `?diag=1`-only diagnostic when this table was written; `FIX-219`/`AUDIT-217` (2026-09) made it real for ordinary families. It is a **second, independent answer to "what does Lincoln know,"** running alongside `skillSnapshots` — not a replacement for that row's "what to teach next" authority, since a guided eval or quest reads this store's synthesized frontier to decide what's next while `skillSnapshots` is what a scan/eval/manual edit records as achieved. **The two stores have no general cross-store reconciliation today** — `core/foundations/evalModelSync.ts`'s `needsReconcile` flag is an *internal* `learnerModels` mechanism: it fires only when a guided eval's read disagrees with a parent **attestation already stored inside `learnerModels`** (`hasAttestation(prev) && read.state !== fromState`), and `progress/conceptOverride.ts` resolves it by comparing that eval evidence against the stored attestation, both learnerModels-internal reads — it never reads `skillSnapshots`. When this store and `skillSnapshots` disagree for some other reason, nothing here reconciles it; that gap is the live seam this row exists to name, not something already closed. |
