...
| **Concept frontier / synthesis** — the synthesized "what matters next" read across a child's foundations concepts | `learnerModels/{childId}` | The direct Firestore writers only (a caller/wrapper is not listed separately, to avoid an ever-growing, never-complete list of entry points): `evalModelWriteback.ts`, `questModelSync.ts`, `writeReviewAction.ts`, `workbookPositionSync.ts` (reached by both `activityConfigWrites.ts` and `useScanToActivityConfig.ts`'s two scan paths), `bootstrapLearnerModel.ts` (create-only; reached by `useFoundationsBootstrap.ts` and `FoundationsDiagPanel.tsx`'s diagnostic reseed), `stuckRetestQueue.ts`, `learnerSynthesis.ts` (merge-writes the `synthesis` field) | Foundations tab, planner focus line (`FoundationsFocusLine`), Shelly chat, weeklyReview (via `synthesizeIfStale`) | **Added 2026-09-20 (`DOC-26`), filed by `ARCHITECTURE_AUDIT_2026-09-13.md` §2.1; corrected twice on Codex review of the same PR.** Round 1: the first draft named `strandSessionWrites.ts`, whose own header states *"No XP, no skillSnapshots, no learnerModels, no day-log write"*; it never touches this store and is removed, and the confirmed real writers `learnerSynthesis.ts`/`stuckRetestQueue.ts` were added. Round 3: the list still mixed caller wrappers with direct writers and missed two more callers of the latter (`useScanToActivityConfig.ts` calling `workbookPositionSync.ts` directly, `FoundationsDiagPanel.tsx` calling both `workbookPositionSync.ts` and `bootstrapLearnerModel.ts`) — the cell now names only the direct writer modules, with their known callers noted in parenthesis rather than listed as peers, since a list of callers alone is never complete. A store the 2026-05-30 ruling did not name because the Learner Model was still a `?diag=1`-only diagnostic when this table was written; `FIX-219`/`AUDIT-217` (2026-09) made it real for ordinary families. It is a **second, independent answer to "what does Lincoln know,"** running alongside `skillSnapshots` — not a replacement for that row's "what to teach next" authority. **It is not what a guided eval or quest reads to pick what's next**: `evalModelWriteback.ts` reads the model only to merge a completed eval's findings in *after* apply, and `useQuestSession`'s `selectQuestTargets` reads only the model's `openQuestions`, never `synthesis.whatMattersNext` — the synthesized frontier's actual readers are exactly the three named above (Foundations tab, planner, Shelly/weeklyReview). **The two stores have no general cross-store reconciliation today** — `core/foundations/evalModelSync.ts`'s `needsReconcile` flag is an *internal* `learnerModels` mechanism: it fires only when a guided eval's read disagrees with a parent **attestation already stored inside `learnerModels`** (`hasAttestation(prev) && read.state !== fromState`), and `progress/conceptOverride.ts` resolves it by comparing that eval evidence against the stored attestation, both learnerModels-internal reads — it never reads `skillSnapshots`. When this store and `skillSnapshots` disagree for some other reason, nothing here reconciles it; that gap is the live seam this row exists to name, not something already closed. |
