...
- **A domain anchor.** A tag whose leading segment declares a domain may not
  resolve outside it. The single exception is declared with its reason: a
  `writing.*` spelling tag may reach a `reading.*` node, the lane
  `deriveWorkingLevelMastery` already permits ("spelling a CVC word implies you
  can decode it"), and it is one-directional. **The anchor changes no answer for
  any of the 177 tags below** — it is a guard against the tags the universe does
  not yet contain, `writing.paragraph` being exactly one of those.
