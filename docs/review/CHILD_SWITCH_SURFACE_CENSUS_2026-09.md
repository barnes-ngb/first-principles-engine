...
| `src/features/progress/AddActivityDialog.tsx` | a typed new activity — name, type, subject, minutes, cadence, position | `activityConfigs` — a new row via `onAdd` | Selector (Curriculum) | **RESET** — the form did not reset on a `childId` change and `handleAdd` stamps the live prop, so a workbook typed out for one boy was created on his brother's curriculum, where it then plans every day. Cleared during render through `addActivityOwnership.ts`, whose emptiness rule reads **every one of the nine fields** (asserted to be all of them, so a tenth fails the test rather than being silently discarded — the UX-336 `DEFAULT_AWARD_TYPE` trap). `UX-354`'s reassign door exists because there was no way to move such a row; this stops writing it. Fixed by FIX-232 (UX-335) | — |
