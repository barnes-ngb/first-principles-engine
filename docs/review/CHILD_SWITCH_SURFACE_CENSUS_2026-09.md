...
| `src/features/monthly-review/MonthlyBooksTab.tsx` | a generate-dialog flag; the list follows the active child | delegated to `GenerateNowDialog` | Selector (Review) | **SAFE** — the list filters live on `activeChildId`; the only write is the modal dialog’s, which carries its own picked child and refreshes its default when opened | — |
