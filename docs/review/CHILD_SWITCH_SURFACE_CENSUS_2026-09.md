...
```
source files scanned (non-test, under src/, excluding src/test/): 790
files reading useActiveChild: 73
candidates: 95 (hook arm 43, prop arm 52)
files rendering an in-page <ChildSelector>: 0
feature files referencing setActiveChildId (any in-page child control): 3
...
Owner, 2026-09-13, from the first post-deploy test: *"There are now as many as four locations to
choose a child. I like the chip drop-down in the header as the primary source; remove the others.
Keep the actual profile change between parent and child."* That reverses the 2026-09-09 *"the in-page
selectors stay"* recorded in §7 — deliberately, and by the person who made it, after living with the
switcher `FIX-231` turned on.

**Two counts moved and the candidate set did not.** `files rendering an in-page <ChildSelector>` went
**9 → 0** (the component is deleted) and `setActiveChildId` in feature files **15 → 3**. `candidates`
held at 95 and `census rows` fell 96 → 95, which is the one deleted row: `ChildSelector.tsx` itself,
whose SAFE row now has no file. Every remaining surface is unchanged in what it holds and what it
writes — only the *Reachable by a switch today?* column moved, from *Selector (its own
`ChildSelector`)* to **Shell only**, on the nine pages that had one. **That is a narrowing of reach,
not a widening**, which is why no verdict or severity changed: a surface reachable by two routes is
now reachable by one, and the class this registry bounds is unaffected.

**Three surfaces were pickers without being `ChildSelector`s, and the sweep's own grep could not see
them** — `ArmorTab` (a `Button` pair), `MyAvatarPage` (a themed `Box` row, which also keyed its
styling on a literal `child.name === 'lincoln'`) and `AvatarAdminTab` (a `Chip` row that is also the
duplicate-mark and per-profile-delete list, so the chips stay and only the `onClick` went). Counting
the component would have left the owner's *"four locations"* an undercount on those screens, so
`src/test/oneChildControl.source.test.ts` states the rule over the **act** — a click that calls
`setActiveChildId` — rather than over the component.

**The three feature files still calling `setActiveChildId` are consequences, not choices**, and are
named in that test so a fourth cannot join them quietly: deleting the avatar profile you were on
falls back to one that exists (`AvatarAdminTab`), opening a monthly book follows to that book's child
(`MonthlyReviewReaderPage`), and resuming a draft follows to the draft's child
(`BookshelfPage`, FEAT-188 `draftOwnership`).

**The two counts now read code rather than raw text.** Each removal above is explained by a comment
that names the control it replaced, and a raw scan counted those three sentences as three surfaces —
the `[ledger-shape]` failure in reverse, a scan satisfied by prose. `stripComments` in
`src/test/childSwitchSources.ts` is shared by the script and the guard so they cannot answer
...
|---|---|---|---|---|---|
| `src/components/ChildSwitcherChip.tsx` | a menu anchor element and, since `UX-425`, an add-child dialog flag | nothing itself — a source scan pins that it writes no Firestore document; the dialog it hosts writes `children`, behind a form and a Save | It **is** the switch, and since `UX-425` the ONLY one: the shell's two sites, one per viewport (the drawer passes `showChildChip={false}`). `ContextBar`'s chip and the ten in-page `ChildSelector`s are gone | **SAFE** — the anchor is a DOM node for a menu, the dialog boolean means the same thing whichever child is active (the `ChildSelector` row this replaces said so first), and the chip's whole job is to call `setActiveChildId` | — |
...
| `src/features/avatar/AvatarPhotoUpload.tsx` | a staged photo preview awaiting "extract features" | `avatarProfiles/{childId}` + one paid `extractFeatures` call against the art quota | Selector (Hero Hub's own child chips) | **RESET** — the staged photo survived the switch while `handlePhotoTransform` read the live `childId` prop, so one boy's face seeded his brother's `characterFeatures` and spent his art quota doing it. The preview is now dropped during render through `avatarChildSwitch.stagedPhotoSwitchNotice`, and the notice says the budget was **not** spent — the one fact the screen cannot show. A transform already in flight needed no rule: it is a `useCallback` over `[familyId, childId, …]`, so it carries its child in its own closure. Fixed by FIX-232 (UX-331) | — |
| `src/features/avatar/MyAvatarPage.tsx` | tuner state (`localProportions`, `heroAnimationTuning`, `armorDebugValues`) and a captured `screenshotData` | `avatarProfiles`, `dailyArmorSessions`, `artifacts`, `xpLedger` (via `addXpEvent`) | Selector (its own child chips, line 1361) | **RESET** — `screenshotData` was the half that crossed a rail: `saveToPortfolio` stamps an `artifacts` document with the live `childId` and the live child's **name** while the PNG is the previous boy's character. Cleared during render with the tuner and the two debug overlays; `localProportions` is left to the profile listener, which already re-seeds it, because a second writer there would race the first. `xpLedger` is untouched — `addXpEvent` is still the same event, the same **5** and the same dedup key, pinned with a positive control (`DOC-25`). Fixed by FIX-232 (UX-332) | — |
...
| `src/features/books/SketchScanner.tsx` | a captured sketch, cleanup marks and its cleaned/fancy versions | `stickerLibrary` — `childId: null` by construction | Shell only | **BIND** — capture keeps its initial label and explicit local For picker across header child changes, including paid Fancy results; save snapshots family/group/profile before awaits. Close, retake and family replacement invalidate late preparation. Adjust cleanup is locked after the single cleaned anchor is saved (FIX-246). | — |
...
| `src/features/evaluate/EvaluateChatPage.tsx` | a live evaluation session — messages, findings, recommendations | `evaluationSessions`, `skillSnapshots`, `childSkillMaps`, `learnerModels`, `hours` | Shell only — its own `ChildSelector` went with `UX-425` | **GATE** — the child-change effect sets `initializing` synchronously and the entire body, Apply included, renders behind it until the new child's sessions land | — |
...
| `src/features/evaluation/SkillSnapshotPage.tsx` | the loaded `SkillSnapshot` and open section IDs | `skillSnapshots/{activeChildId}` | Shell only — its own `ChildSelector` went with `UX-425` | **GATE** — `snapshotChildId` is compared during render and `snapshot` is nulled on a change; the open section IDs reset, the page renders *"Loading skill snapshot…"*, and every editor below it is unmounted. Collapsing a section for the same child only hides it and preserves drafts | — |
...
| `src/features/progress/AddActivityDialog.tsx` | a typed new activity and pending save | `activityConfigs` — a new row via `onAdd` | Selector (Curriculum, Planner) | **RESET** — the form did not reset on a `childId` change and `handleAdd` stamps the live prop, so a workbook typed out for one boy was created on his brother's curriculum, where it then plans every day. **Two runs fixed this independently and the merge kept the better half of each** (PR #1840 and `FIX-232`): the reset is `AddActivityForm` **keyed by `childId`**, so every field clears by construction and a field added later cannot be forgotten — stronger than any list of setters — while an already-started save retains its original props and the unmounted form cannot close its successor. The **notice** is the question a remount cannot answer, *was there anything to lose?*, so the form reports its own dirtiness to the wrapper (the only thing outliving the remount) and `addActivityOwnership.ts` owns both halves: an emptiness rule reading **every one of the nine fields**, asserted to be all nine so a tenth fails the test rather than being silently discarded (the `UX-336` `DEFAULT_AWARD_TYPE` trap), and a sentence that names both boys. An untouched dialog raises nothing — a notice on every switch is one nobody reads by the time it matters. Fixed by FIX-232 (UX-335) | — |
...
| `src/features/progress/CurriculumTab.tsx` | staged scan pages, a strand-session draft, a certificate confirm | `activityConfigs`, `scans`, `skillSnapshots`, `childSkillMaps`, `artifacts` | Shell only — its own `ChildSelector` went with `UX-425` | **RESET** — `stagedChildId` is stamped at staging time, a switch drops the batch with one line saying so, the write path guards on it again, and a switch **during** a batch discards the completion (UX-275) | — |
| `src/features/progress/DispositionProfile.tsx` | an inline narrative override being typed, the AI `result` | `children/{activeChildId}.dispositionOverrides` | Shell only — its own `ChildSelector` went with `UX-425` | **RESET** — the child-change effect clears `overrides` and `editingKey`, and `handleEditSave` returns early without an `editingKey`, so the cross-child write is unreachable | — |
...
| `src/features/progress/FoundationsTab.tsx` | an open concept drawer, an override selection | `learnerModels/{activeChildId}` via `writeReviewAction` | Shell only — its own `ChildSelector` went with `UX-425` | **SAFE** — the override refuses when `action.childId !== activeChildId`, and the action is built from the same `childId` the drawer was opened under | — |
...
| `src/features/settings/AvatarAdminTab.tsx` | typed XP / diamond amounts, a reason, pending regenerations | `avatarProfiles`, `xpLedger`, `dailyArmorSessions`, `children` | Selector (its own child chips) | **RESET** — `ArmorTab`'s shape (`UX-336`) on the admin surface, holding more: two typed amounts, a shared reason, and **two** confirm dialogs whose own buttons call handlers reading the live `activeChildId`. Both are closed with the draft — *Deduct diamonds* names the child in its text, and *Regenerate base character* spends a paid image call — and `feedback` goes too, since a green "Awarded 10 XP" under the new boy's name reads as a receipt for him. It **prevents** a write rather than changing one, so `DOC-25` attribution-only: `addXpEvent`'s arguments, the 200-diamond cap and the positive-amount guard are pinned unchanged with a positive control. Fixed by FIX-232 (UX-341) | — |
...
|---|---|---|---|---|
| `PlannerChatPage`'s model inputs — `snapshot`, `weekPlan`, `hoursPerDay` | the previous child's skill snapshot | nothing directly; they reach the planner prompt | **RESET** — the listener's `if (snap.exists())` made it replace-only, so a child with no snapshot document, or one whose read failed, was planned against his brother's working levels and priority skills. Both arms now clear: a missing document and a **failed read** alike, which is deliberate rather than lazy — the only two outcomes available are "this child's snapshot" and "none", and the wrong child's is not one of them. The other two values were checked and need nothing: `weekPlan` is keyed on `weekRange.start` alone (a week document is the family's), and `hoursPerDay` re-derives from `dailyRoutine`, which is already filtered by `activeChildId`. Fixed by FIX-232 (UX-334) | — |
| `DispositionProfile`'s cached AI narrative — `result` | one child's generated disposition narrative | nothing — display only | **RESET** — `overrides` and `editingKey` were already cleared, which is why the cross-child *write* was unreachable; `result`, `cacheAge` and `aiGeneratedAt` were only ever **replaced**, and only when the new child happened to have a fresh cache, so one boy's paragraphs were read under his brother's name. All three now clear, and a `cancelled` flag closes the same defect's second door — `getDoc` is a round trip, and a switch during it let the previous child's cache land after the clear. **No notice**: the narrative is a 24-hour Firestore cache, nothing a person made is lost, and the honest fallback is this section's own Generate door. Fixed by FIX-232 (UX-337) | — |
...
**Since `UX-425` the shell is the ONLY rule.** There is one child control in the product — the
`ChildSwitcherChip` in `AppShell`, rendered once per viewport (the mobile header below 900px, the
desktop sidebar above; the mobile drawer shares `NavContent` and passes `showChildChip={false}`,
because the header's chip is visible above the open drawer). A surface that needs to say *whose*
record it is names the child in its own heading or renders the shared `ActiveChildLine`; it does not
offer a way to change him. The **profile** switch between parent and child (`ProfileMenu`) is a
different thing and is untouched, by the owner's own sentence.
