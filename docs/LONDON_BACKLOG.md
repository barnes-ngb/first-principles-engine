...
| **Kid Today — extra-activity logger** | Ready | — | `KidTodayView.tsx:666` → `KidExtraLogger.tsx` | Renders unconditionally ("I did more!" capture). Safe: low-friction, photo/voice capture, no reading load. **Re-verified 2026-09-03: still Ready** — `KidTodayView.tsx:857-863` → `KidExtraLogger.tsx:95-118` (unconditional). Copy was Minecraft-framed for both kids (`⛏️ I Did More Mining!`, `Did extra work on your tablet? (Reading Eggs, Math App, Typing) Log it here!`, `:95-98`) — P3 copy, §Audit 2026-09. **FIXED 2026-09-04 (FEAT-186):** `⭐ I Did More!` / `Did more work today? Add it here.` / `⭐ Add More Work` / `💎 Save It!`, and the save error no longer asks a six-year-old to check his connection. **The preset chips are deliberately unchanged:** their labels are written into `days.checklist[].label`, so they are a stored data shape, not copy — the tablet-app assumption stays filed. |
...
| **Story Guide (`/books/story-guide`)** *(added by FEAT-183)* | **RETIRED (FEAT-187, 2026-09-04)** | — | `src/features/books/StoryGuidePage.tsx:57`, `useStoryGuide.ts:39,47,100-102,256` | The guided five-question story wizard. London gets the younger question set (`Who is in your story?` / `Where do they live?` / `What happens one day that's surprising?` — sight-word level, with TTS read-back and a voice answer path), and a `storybook` theme on the brief. **The set was selected by name** (`useStoryGuide(isLincoln)` → `LINCOLN_QUESTIONS : LONDON_QUESTIONS`) — B5 in §Audit 2026-09. **FIXED 2026-09-03 (FEAT-183):** the hook takes an `AgeGroup` and the constants are named for what they are (`OLDER_QUESTIONS` / `YOUNGER_QUESTIONS`); the theme at `:256` keys the same way. Same two sets, same content — London's flow is unchanged. **RETIRED 2026-09-04 (FEAT-187):** the wizard is deleted (one "Make a book" door, two choices — "Write it myself" or "Make one with Shelly"); `/books/story-guide` redirects to the shelf. It was reachable only from a buried link inside the AI tab, and on every axis it sat behind the Generate chat London actually meets — that chat has the style picker, the FEAT-176 reading level, the Whisper voice module and a finish that waits for a tap. **What London loses is the scaffolding:** five questions with hints instead of one blank idea box. Preserved verbatim (both sets, all hints) in `docs/DESIGN_STORY_GENERATION_V2.md` §12.3 — if a 6-year-old turns out to need the prompts, they come back as a way to FILL the chat's idea box, never as a second generator. Log it here if it comes up in use; don't rebuild it speculatively. |
...
| **Kid Today — `Show your work!` photo capture** *(reconciled — the AI scan path)* | Ready — **photo + description only from a kid (FEAT-184)** | — (decided 2026-09-03: a kid capture seeds nothing) | `KidChecklist.tsx:280-291` → `KidTodayView.tsx:440-454,1109-1136` → `useUnifiedCapture.ts:120-121,365-376` | The per-item capture runs the same AI scan pipeline as the parent view (`useScan` → one LLM call per photo, uncapped) and, when the page is recognised as a worksheet/textbook/test, **creates or advances an `activityConfigs` doc, derives `skillSnapshots.workingLevels`, folds the position into `learnerModels` and updates `childSkillMaps`** (`:326,353` → `useScanToActivityConfig.ts:128,138,193,277`), then on a detected blocker **merges `skillSnapshots.conceptualBlocks`** (`:373`) plus a `scans` doc — all with no `canEdit` / `isChildProfile` guard. The dialog shows `Saving your work...` for the whole round-trip. **UX-151 (P1)** in §Audit 2026-09. **FIXED 2026-09-03 (FEAT-184):** one gate on the lane (`useUnifiedCapture`, `invariantWritesAllowed = !isChildProfile`, read inside the hook): a kid capture still runs the one scan and keeps the photo, with the scan's description on the artifact's `contentNote`; the workbook advance, `workingLevels`, `conceptualBlocks`, `learnerModels` and `childSkillMaps` writes run only for a parent actor. A kid photo read as a worksheet stamps `pendingScanId` and parent Today shows "Review this ▾" on the existing analysis chip. The dialog copy is Run C's. |
| **My Stuff (`/records/portfolio`)** *(kid nav — REMOVED 2026-09-04)* | **Not offered** (nav entry gated) | A kid-shaped gallery (filed under Not-built) | `src/app/AppShell.tsx:47`, `src/features/records/PortfolioPage.tsx:289-360,617-620` | The kid nav's `My Stuff` opens the parent Portfolio: `Portfolio / Demo Night Highlights`, `Year` / `Month` selects, `Search by title`, `Subject` / `Type` filters and a markdown export button. Read-only and harmless, but nothing on it is for a 6-year-old — P2 copy/shape. (UX-81 already notes the Today card `📸 My Stuff` is a different destination.) **FIXED 2026-09-04 (FEAT-186, owner decision):** the kid nav entry is gone. The **route is untouched** — a parent still reaches the Portfolio from Records and a typed URL still resolves — so this is the nav declining to advertise a destination, not a guard. A kid-shaped gallery is a design and was deliberately not built in a wording run; it is filed as a candidate in the Not-built band. Kid Today's own `📸 My Stuff` card is a different destination and stays, which also closes UX-81's two-destinations-one-name item for a kid. |
...
| **Settings (via the profile menu)** *(gated 2026-09-04)* | **Not reachable by tap** | — | `src/components/ProfileMenu.tsx:85-88,174-179`, `src/features/settings/SettingsPage.tsx:87,160-200` | Not in the kid nav (`parentOnly`, `AppShell.tsx:37`) and not route-guarded; the profile pill's `Settings` item was ungated (UX-76), so a kid reached the `General` tab — a `Theme` select and the `AI Features` switches (`localStorage`, this device only). **FIXED 2026-09-04 (FEAT-186):** the menu item is gated on `isParentProfile` — capability, the same answer `AppShell` uses to pick the kid nav — so the switches are unreachable for a kid without a second gate on them. `/settings` still has no route guard by design (nothing kid-facing links to it). The **profile switcher is deliberately still ungated**: locking a kid's device out of the `Parents` profile is the family's own trust model, a separate decision. UX-76's two presentation notes (the mobile-hidden pill label, the duplicate child chip) are not access and stay open under UX-81. |
...
- **Ready:** 24 — Kid Today checklist, XP/diamonds bar, extra-activity logger, greeting/celebration tone,
  ...
  UX-151 — **fixed by FEAT-184**), Barnes Bros. Most are age-independent or already London-tuned; the
  bolded ones are Ready with a P1 attached. (`My Stuff` left this band on 2026-09-04 — see below.)
- **Hold-until-tuned:** 1 — Knowledge Mine. **The hold has a hole (UX-150):** applying the London
  defaults opens it. **Closed 2026-09-03 (FEAT-184): the gates ignore the starter shape.**
- **N/A:** 1 — Teach-back (relationship-gated on the kid side; name-gated on the parent side, B13).
- **Reachable but untuned / URL-only:** 1 — Ask AI (typed URL only). **Settings left this band 2026-09-04
  (FEAT-186):** the profile-pill item is gated on `isParentProfile`, so it is no longer reachable by tap;
  the route is still unguarded, which is by design (nothing kid-facing links to it).
- **Not offered:** 1 — **My Stuff** (`/records/portfolio`). The kid nav entry was removed 2026-09-04
  (FEAT-186, owner decision) because it opened the parent Portfolio. The route is untouched.
- **Not-built:** 1 — **a kid-shaped "My Stuff" gallery** (FEAT candidate, filed 2026-09-04). What a
  six-year-old would need where the parent Portfolio is now absent: his own photos and recordings, newest
  first, big tiles, no filters, no selects, no export, no search — the shape `Books About Me`
  (`KidBooksAboutMePage`) already proves works for him, pointed at `artifacts` instead of
  `monthlyReviews`. Read-only, no paid call, no write. **Not built speculatively** — it is a design, and
  Run C was a wording run; it needs its own assignment. (The London learner profile shipped 2026-06-20,
  FEAT-34.)
...
### FEAT-184 (2026-09-03) — Run B: the hold holds, a kid's photo writes nothing it should not, and the last two kid-reachable art doors are capped

Owner decisions (Nathan, 2026-09-03), recorded inline in the run-prompt and applied here without a stop:

- **The Knowledge Mine gate ignores starter skills — a starting frame is not calibration.** Both
  `hasReadingCalibration` / `hasMathCalibration` drop any priority skill that still has the exact
  starter shape (`isStarterSkill`: a template tag at `Emerging` / `NotYet`, matched against the union
  of every template in `childDefaults.ts`) before counting. "Load Starter Defaults" no longer opens the
  Mine; the first quest, eval or scan that upgrades a starter row does. The `⛏️ Start Mining` row on Kid
  Today is gated by the same predicate as the Hero Hub tile. **The register's Hold row is now a real hold.**
- **A kid's "Show your work!" keeps the scan, keeps the photo, loses the silent writes.** The one paid
  scan call is worth it (most of what the boys photograph is builds, and the description — the FEAT-141
  `contentNote` — is the substrate for a future "describe the build → follow-up learning" idea). The
  workbook advance, `workingLevels`, `conceptualBlocks`, `learnerModels` and `childSkillMaps` writes run
  only for a parent actor — one gate on the lane in `useUnifiedCapture`. A kid photo read as a worksheet
  leaves the parent a "Review this ▾" chip on parent Today, on the analysis panel that already existed.
- **Workshop and the avatar photo count against the weekly art counter.** Same doc as stickers, books and
  kits — not a second allowance. A game's pictures are reserved as one batch and refused whole when the
  week cannot cover them (the game is still made, with no pictures). The three Workshop text calls are
  not art and stay uncapped. The Shelly-chat image generator is the last uncapped door a kid can reach.

Every gate is UI, on capability, never a name. `firestore.rules` untouched — **ARCH-10 remains open;
the kid-write table below is its input.** Run C (copy and shape) is next.

### FEAT-186 (2026-09-04) — Run C: the words a six-year-old meets on his own screens

The last of the three runs the 2026-09 audit proposed (name-gates → holes and spend → copy). **Text and two
nav entries only** — FEAT-154 rules: no new props, handlers, surfaces, reads or writes, nothing near hours /
XP / diamonds / compliance / prompts / `firestore.rules` / the art quota. Capability, never a name.

**The decision that carried the batch: `quest` is Lincoln's word.** His must-do card is titled
`⛏️ Daily Quests`; London's is `Must Do`. But the *shared* strings — the ones neither branch forks — had all
been written in his framing: `{n} quests to go!`, `🔒 Complete {n} more quests`, `🔒 Finish quests first`,
`Complete {n} more quests to unlock!`, `Light day today. Just {n} quests!`. So the surface that is supposed to
be age-blind was quietly speaking to one child. The replacement noun is **`job`**: it is the app's own kid
word already (Dad Lab's `Your Job` / `{name}'s Job`, both marked ✓ by this audit), and it decodes at London's
level as CVC + s, which `quests` does not. Lincoln's Minecraft *titles* keep their vocabulary; only the copy
both kids read changed. UX-81's rule — a noun on every count — is preserved everywhere.

Where a string had been **forked by name**, the fork is gone rather than doubled: the Choose-section lock was
`Complete your quests to unlock crafting!` / `Complete your must-do items to unlock choices!`, two spellings
of one sentence, and both were above a six-year-old (`complete`, `unlock`, `choices`, `items`). It is now one
line, `Do your must-do jobs first.`, which is true and readable for either kid — the run-prompt's rule that
the shorter wording serves both.

**The bar is now shared, not copied.** FEAT-178's readability proxy moved from
`books/__tests__/artHelpContent.test.ts` into `src/test/kidReadability.ts`; the art-help suite imports it and
its assertions are unchanged. `expectKidWording` (≤ 8 words, nothing over two syllables by the vowel-group
proxy) applies to every kid string; `expectKidLine` adds the full stop and applies only to sentences — a chip
label and a bare count are not sentences, and demanding a period of them would make the copy worse. The proxy
is still deliberately cheap and still says so: the real classifier (`functions/src/ai/storyDecodability.ts`)
is server-only, so this is a floor on carelessness, not a reading measurement.

`src/test/kidLockWording.test.ts` is the deliverable that keeps the next run honest. It walks all 18 strings
and asserts each is **still in the file that renders it** *and* passes the bar — the bar alone would pass a
string nothing renders, and a presence check alone would pass a string no kid can read — plus a
retired-phrasings scan over comment-stripped source, so an old wording cannot creep back somewhere else. It
reads source rather than rendering, because mounting `KidTodayView` or `KidLabView` costs a Firestore harness
apiece and would make the suite about mocking rather than about words; hoisting the strings into a copy
module was the other option and was out of scope under the text-only rail.

**The two nav doors.** `My Stuff` opened `/records/portfolio` — the parent Portfolio, with Demo Night
Highlights, Year/Month selects, a title search, Subject/Type filters and a markdown export. The owner decision
was to drop the kid nav entry rather than build a gallery inside a wording run; the gallery is filed as a
Not-built candidate above. The profile pill's `Settings` item is now gated on `isParentProfile`, which puts
the General tab's AI Features switches out of a kid's reach without needing a second gate on the switches
themselves. Neither change touches a route: both are the nav declining to advertise a destination.

**What was left, and why.** The armor gate's **Ephesians 6:11** quotation stands verbatim — a quoted verse
cannot be shortened without misquoting it, and UX-73's real complaint is the screen's *shape* (a whole day
replaced by one control), which is a design; **UX-73 stays open**. The extra logger's **preset chips**
(`📖 Reading Eggs`, `🔢 Math App`, …) are written into `days.checklist[].label`, so they are a stored data
shape rather than copy. The **armor-forge tier locks** (`Complete Wood (3/6) & earn 250 XP`,
`Locked — earn more XP to forge this piece.`, `getTierLockReason`) carry XP math and drive the forge CTA —
the one rail this run was told to stay clear of. Dad Lab's **framework step chips** (UX-89) are tappable but
track nothing, which is behaviour, not text.
...
**How he gets anywhere:** the kid nav (`AppShell.tsx:42-50`) was `Today · Knowledge Mine (hidden) · My Books ·
...
**As of 2026-09-04 (FEAT-186) two of those doors are closed:** `My Stuff` is gone from the kid nav (it opened
the parent Portfolio) and the profile pill's `Settings` is parent-only. Both routes still exist and are still
unguarded — this is the nav declining to advertise a destination, not a new gate, and the kid-write table
...
8. **P2 — Settings is one tap from the profile pill** (UX-76, still open) and shows AI-feature switches to a
   kid (`SettingsPage.tsx:160-200`). **FIXED 2026-09-04 (FEAT-186):** the menu item is gated on
   `isParentProfile`; UX-76's access half is closed, its two presentation notes move to UX-81.
9. **P2 — `My Stuff` is the parent Portfolio** (`AppShell.tsx:47` → `PortfolioPage.tsx:289-360`): filters,
   ...
   **FIXED 2026-09-04 (FEAT-186, owner decision):** drop the nav item; the route stays. The kid gallery is
   filed as a Not-built candidate rather than built inside a wording run.
10. **P2/P3 — the gate-and-lock copy is above his level everywhere it matters** (`Complete your must-do
    ...
    **FIXED 2026-09-04 (FEAT-186):** 18 strings, all held by `src/test/kidLockWording.test.ts` against the
    shared FEAT-178 bar. **Two things were left, on purpose:** the Ephesians 6:11 quotation (a verse cannot be
    shortened without misquoting it — **UX-73 stays open**, and its real complaint is the screen's shape), and
    the armor-forge tier locks (`Complete Wood (3/6) & earn 250 XP`, `getTierLockReason`), whose strings carry
    XP math and drive the forge CTA — the one rail Run C was told to stay clear of.
...
- **Run B — the holes in the hold and the unsupervised spend (decisions first, then wiring).** ✅ **Shipped as FEAT-184 (2026-09-03).** UX-150
  ...
  ARCH-10 input.
- **Run C — copy and shape.** ✅ **Shipped as FEAT-186 (2026-09-04).** The lock/gate wording pass (#10), the
  `My Stuff` decision (#9), UX-76's Settings gate (#8), the Minecraft-neutral extra logger, Banner Rally and
  Watch captions, Dad Lab's raw `labType` chip. Text only, FEAT-154 rules (no new props, handlers, reads or
  writes) — the two nav entries are the only non-string change, and neither is a guard. See the FEAT-186
  section below.
