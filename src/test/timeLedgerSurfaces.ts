...
const WRITE_VERBS = [
  ...
  // Every guarded day writer `dayWriteGuard.ts` exports. `updateDayLogGuarded`
  // was missing until `UX-404` added the first file that reaches `days` through
  // a guarded writer alone, and `patchDayChecklistGuarded` is that run's own new
  // one — a rule that cannot see a writer classifies its callers as READERS, and
  // a census claiming a write lane does not write is worse than no row.
  /\b(setDayLogGuarded|updateDayLogGuarded|deleteDayLogGuarded|mergeDayLogGuarded|patchDayChecklistGuarded)\s*\(/,
