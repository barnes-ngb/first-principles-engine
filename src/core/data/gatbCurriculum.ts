// ── GATB Scope & Sequence Reference Map ─────────────────────────
// Static curriculum data for Good and the Beautiful (GATB) Language Arts
// and Math, Levels K–2. Used by the AI context pipeline to enrich
// planner and Shelly Chat prompts with covered/upcoming skills.
//
// Source: GATB published scope-and-sequence summaries and TOC pages.
// Grouped into ~10-lesson units. Not every individual lesson is listed —
// the AI uses this to answer "what has the child covered and what's next?"

export interface CurriculumUnit {
  /** Lesson range start (inclusive) */
  lessonStart: number
  /** Lesson range end (inclusive) */
  lessonEnd: number
  /** Primary topic */
  topic: string
  /** Skills practiced in this unit */
  skills: string[]
  /** Phonics patterns introduced (LA only) */
  phonics?: string[]
  /** Sight words introduced (LA only) */
  sightWords?: string[]
  /** Math concepts (Math only) */
  mathConcepts?: string[]
}
...
/**
 * Given a GATB curriculum key and current lesson number,
 * return the skills covered so far and what's coming next.
 */
export function getGatbProgress(curriculumKey: string, currentLesson: number): {
  ...
  for (const unit of level.units) {
    if (unit.lessonEnd < currentLesson) {
      ...
      // Current unit — count skills as in-progress (still add to covered)
      currentUnit = unit
      coveredSkills.push(...unit.skills)
      if (unit.phonics) coveredPhonics.push(...unit.phonics)
    } else {
