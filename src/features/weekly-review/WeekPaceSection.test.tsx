...
// ── Before the Saturday-evening cron has fired (UX-219 / UX-263) ───────────
...
describe('the Saturday state — the week is named before its review exists', () => {
  ...
  it('never promises Saturday evening to a review that exists without a snapshot', () => {
    ...
    expect(container.textContent).not.toMatch(/saved Saturday evening/)
    // And it makes no other claim about coverage either.
    expect(container.textContent).not.toMatch(/rate needs two|lesson/i)
  })

  it('still promises Saturday evening after a parent answers on Saturday', () => {
    ...
    expect(container.textContent).toMatch(/saved Saturday evening/)
  })
