 ...
 *
 * **It promises a date, so it is shown only where that promise is true**
 * (Codex round 1, P2): keyed on the ABSENT REVIEW DOCUMENT, never on a missing
 * snapshot. A review can exist with no `curriculumPositions` — the server omits
 * the field when the child has no positioned workbook config, and when the
 * config read throws — and in both cases the cron has already run and nothing
 * more is coming. Shown there, this sentence would be a promise that never came
 * true, repeated every week. See the guard in `WeekPaceSection`.
 */
