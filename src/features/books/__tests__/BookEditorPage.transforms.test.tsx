...
describe('BookEditorPage with real PageEditor, gesture component, useBook and history', () => {
  ...
  it('reorders one sticker without changing a positionless photo, and can undo/redo that order', async () => {
    ...
    const row = screen.getByText('Synthetic art').closest('[data-layer-id]') as HTMLElement
  ...
  it('centers a saved off-page picture once and Undo restores only its geometry', async () => {
    const position = { x: 140, y: -90, width: 40, height: 30, rotation: 75, flipH: true, flipV: false, zIndex: 3 }
    m.source.pages[0].images[0].position = position
    render(<StrictMode><BookEditorPage /></StrictMode>)
    await screen.findByAltText('Synthetic art')
    fireEvent.click(screen.getByRole('button', { name: 'Show layers' }))
    fireEvent.click(screen.getByRole('button', { name: 'Select Synthetic art' }))
    expect(m.persist).not.toHaveBeenCalled()
    const center = screen.getByRole('button', { name: 'Center on page' })
    center.focus()
    fireEvent.click(center)
    expect(m.persist).toHaveBeenCalledTimes(1)
    expect(persisted().pages[0].images[0].position).toEqual({ ...position, x: 30, y: 35 })
    expect(center).toHaveFocus()
    fireEvent.click(center)
    expect(m.persist).toHaveBeenCalledTimes(1)
    fireEvent.change(screen.getByDisplayValue('Original story'), { target: { value: 'Later text stays' } })
    fireEvent.click(screen.getByText('Undo'))
    expect(persisted().pages[0].images[0].position).toEqual(position)
    expect(persisted().pages[0].text).toBe('Later text stays')
    expect(screen.getByText('Undo').closest('[role="button"]')).toHaveAttribute('aria-disabled', 'true')
    fireEvent.click(screen.getByText('Redo'))
    expect(persisted().pages[0].images[0].position).toEqual({ ...position, x: 30, y: 35 })
    expect(persisted().pages[0].images[0].url).toBe('synthetic.png')
  })
})
