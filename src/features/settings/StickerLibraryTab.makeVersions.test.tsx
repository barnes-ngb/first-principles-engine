...
it('the settings version door shares the comic request and keeps the saved source group', async () => {
  enhanceSketchMock.mockReset().mockResolvedValue({ url: 'https://example.test/comic.png', storagePath: 'p/comic.png' })
  addDocMock.mockReset().mockResolvedValue({ id: 'comic' })
  ...
  await user.click(await screen.findByRole('button', { name: /make more versions/i }))
  const dialog = await screen.findByRole('dialog', { name: /make another version/i })
  expect(within(dialog).getByText(/Watercolor look/)).toBeInTheDocument()
  await user.click(within(dialog).getByText(/Comic-book look/))
  expect(enhanceSketchMock).not.toHaveBeenCalled()
  await user.click(within(dialog).getByText(/Make it/i))
  await waitFor(() => expect(addDocMock).toHaveBeenCalledTimes(1))
  expect(enhanceSketchMock).toHaveBeenCalledWith(expect.objectContaining({ style: 'comic', transparent: true, familyId: 'family-1' }))
  expect(addDocMock.mock.calls[0][1].theme).toBe('comic')
})
