...
describe('collectHoursContributions (DATA-11)', () => {
  ...
  it('agrees with the functions-side port on the shared PARITY FIXTURE', () => {
    // THE PARITY FIXTURE (FEAT-164). `functions/` cannot import from `src/`
    // (TS6059 `rootDir` + TS2835 node16 resolution), so the monthly review book
    // is generated by a Cloud Function carrying a second implementation of this
    // exact counting rule in `functions/src/ai/tasks/monthlyHours.ts`. This
    // fixture and its expectation are repeated VERBATIM in that module's test
    // (search it for "PARITY FIXTURE"); change one implementation and this pair
    // fails until you change the other.
    //
    // The per-subject map is asserted alongside the total because the book's
    // prose RANKS subjects off it ("Science was his biggest subject this
    // month") — per-subject drift rewrites the narrative even when the totals
    // still agree.
