...
describe('collectHoursContributions (DATA-11)', () => {
  ...
  it('folds a realistic multi-source month into the compliance figure', () => {
    // This corpus is the data half of the retired PARITY FIXTURE (FEAT-164),
    // which used to be repeated verbatim here and in
    // `functions/src/ai/tasks/monthlyHours.test.ts` to pin two hand-kept copies
    // of the counting rule together. ARCH-47 slice 4 gave the rule ONE
    // definition (`functions/src/shared/hoursContributions.ts`) compiled by both
    // projects, so the COMPILER now holds the Records page and the monthly
    // review book together and the fixture's parity role is gone.
    //
    // The corpus survives because its coverage is worth keeping on this side
    // too: it exercises `computeHoursSummary`'s own fold — which this repo owns
    // and the book does not — over a month that touches every branch of the
    // counting rule (block-actuals dedup, a DATA-14 carried item, a
    // checklist-only day, DATA-09 'both' vs sibling attribution, a negative
    // correction, an `hours`-not-`minutes` entry, a zero-minute entry, and
    // another child's records filtered out by the safety net).
    //
    // The per-subject map is asserted alongside the total because the book's
    // prose RANKS subjects off the same numbers ("Science was his biggest
    // subject this month") — a per-subject error rewrites the narrative even
    // when the totals still agree.
    const parityChildId = 'c-lincoln'
    const parityDayLogs = [
      {
        childId: 'c-lincoln',
        date: '2026-08-03',
    ...
    const summary = computeHoursSummary(
      parityDayLogs as unknown as DayLog[],
      parityHoursEntries as unknown as HoursEntry[],
      parityAdjustments as unknown as HoursAdjustment[],
      parityChildId,
    )
    const minutesBySubject = Object.fromEntries(
      summary.bySubject.map((row) => [row.subjectBucket, row.totalMinutes]),
    )

    expect({ totalMinutes: summary.totalMinutes, minutesBySubject }).toEqual(parityExpected)
  })
})

// ─── Reading a malformed document (ARCH-47 slice 4) ─────────────────────────
//
// The counting rule moved to `functions/src/shared/hoursContributions.ts`, which
// reads UNVALIDATED Firestore documents on the Cloud Functions side and so
// narrows its inputs structurally instead of trusting the declared types (that
// directory's rule 3). The app's wrappers keep their typed signatures, so on
// well-typed data — which every app call site's types guarantee — nothing here
// changed and no compliance figure moved.
//
// These cases pin the ONE behavioural delta: where a stored field holds a value
// its type forbids, the rule now yields a defined number instead of propagating
// `NaN` into a total or throwing. Each of these FAILED before the consolidation.

describe('the counting path on a malformed document', () => {
  it('counts the completed checklist items of a day log that has no `blocks` key', () => {
    // Previously threw `Cannot read properties of undefined (reading 'filter')`,
    // taking the whole Records page down rather than mis-counting.
    expect(
      dayLogMinuteContributions({
        childId: 'lincoln',
        date: '2026-01-10',
        checklist: [{ label: 'Read aloud (5m)', completed: true }],
      } as unknown as DayLog),
    ).toEqual([{ subjectBucket: 'Other', minutes: 5, location: 'Home' }])
  })

  it('treats a non-finite `minutes` on an hours entry as absent rather than NaN', () => {
    expect(entryMinutes({ date: '2026-01-01', minutes: NaN } as HoursEntry)).toBe(0)
    // …and still falls through to `hours` when it can.
    expect(entryMinutes({ date: '2026-01-01', minutes: NaN, hours: 1.5 } as HoursEntry)).toBe(90)
  })

  it('counts a non-finite adjustment as zero, so one bad doc cannot NaN the whole total', () => {
    const adjustments = [
      { childId: 'lincoln', date: '2026-01-13', minutes: NaN, reason: 'corrupt' },
      { childId: 'lincoln', date: '2026-01-14', minutes: 30, reason: 'real' },
    ] as unknown as HoursAdjustment[]

    expect(collectHoursContributions([], [], adjustments, 'lincoln').map((c) => c.minutes)).toEqual([0, 30])
    expect(computeHoursSummary([], [], adjustments, 'lincoln').totalMinutes).toBe(30)
  })
})
